...
# 此处为了加载文件时默认生成的array_record文件名称和实际文件相匹配，修改了dataset_name，同时数据集文件夹中dataset_info.json第四行的名称也修改 "RT_1_paper_release"为
# "fractal_fractal_20220817_data_traj_transform_rt_1_without_filters_disable_episode_padding_seq_length_6_no_preprocessor"
dataset_name = "fractal_fractal_20220817_data_traj_transform_rt_1_without_filters_disable_episode_padding_seq_length_6_no_preprocessor"
data_dir = "/LOG/realman/LLM/RT1datasets"

builder = tfds.builder(dataset_name, data_dir=data_dir, file_format='array_record')
builder.download_and_prepare()
loaded_dataset = builder.as_data_source(split=split)
...
load_dataset_path = '/LOG/realman/LLM/RT1datasets'  # 数据集所在目录
# load_dataset_path = 'RT_1_paper_release:1.0.0'
loaded_dataset = tfds.builder_from_directory(load_dataset_path).as_data_source(split='all')
